# Forge Intelligence — Build History

Long-form retrospective archive (renamed from `PLAN.md`). Dated session logs, decisions, incidents, and shipped work — search by date or topic when context is missing. The lean current-state pointer is `WORKING-STATE.md`; the operating guide is `AGENTS.md`.
...
Brian: "promote the Brand Intelligence page from the `strategy` branch to development." The strategy-branch page (`BrandIntelligenceBriefPage`) turned out to be a **dead frontend** — it fetches `/api/strategy/brief/:token`, and that route (plus `/share`, `/compliance`, the 4 deliverable generators beyond PVA/fault-lines, and two tables) existed **nowhere in code** on any branch. STRATEGY.md marked the whole 6-deliverable product "Built ✅"; the code was gone. Rather than ship it dead, traced *how* via git forensics (Brian: "search through the commits").

**The forensic trail:** the full feature — **17 `/api/strategy/*` endpoints** (gap-map, blind-spots, whitespace, pivot GET+POST; share/shares/delete; public brief; compliance GET+POST; compliance-fix; + competitive-intel) — was working at commit **`5b1ac6e9c6`** (2026-04-19 05:45). The WHITEBOARD (line 371-373) documented the kill: on 4/19 19:12, commit `7fd4a9a50e` "sync: port server.js from production" **wholesale-overwrote server.js**, wiping 8 strategy endpoints; a follow-up restored only gap-map; the other 12 handlers (44KB) sat dead 3 days, were hand-restored once, then **lost for good** in the `1873bfb` strategy→main rebuild ("reset to main + restore only the 6 doc/frontend files"). The old strategy lineage survives partially as `origin/Intel` but never carried the 6-deliverable backend. GitHub still served the orphaned `5b1ac6e9c6` by SHA (`git fetch origin <sha>` succeeded), so the complete code was recoverable verbatim.

**Phased restore (Brian's call):**
- **#491 backend** — extracted the 15 handlers (4465–5617 of the April server.js) into a NEW **isolated module `src/server/routes/strategy.js`** (`app.use('/api/strategy', …)`), deliberately NOT back into the monolith: the WHITEBOARD's own post-mortem said a monolith server.js is exactly what let a prod-sync wipe this twice, and named "isolate strategy endpoints into a separate file" as the fix. competitive-intel GET/POST stay inline (they survived the wipe); the router has no `/competitive-intel` path so those fall through. Faithful port — bodies verbatim, only: registration `app.METHOD('/api/strategy/x')`→`router.METHOD('/x')`; model `claude-sonnet-4-20250514`→`claude-sonnet-4-6` (×5); the 5 AI-output `JSON.parse`→`extractJSON` (the 3 DB-value parses — stringify→string-replace→reparse for jsonb writes — kept as-is). Schema: `brand_intelligence` (one row per brand+deliverable, JSONB, UNIQUE(brand_profile_id, deliverable) for the ON CONFLICT) + `brand_intelligence_shares` (token/created_by/brand_name/brand_url/revoked_at) reconstructed from the handler SQL (DDL was never committed — created ad-hoc in prod) as boot-time `CREATE TABLE IF NOT EXISTS`. CI caught the route-inventory guard (15 added, 0 removed — all intentional); regenerated the snapshot.
- **#492 frontend** — dev already carried an orphaned 2-tab `StrategyIntelPage` stub (competitive-intel only, no route/nav, unreachable). Replaced it with the real **6-tab workspace** (Gap Map · PVA · Fault Lines · Blind Spots · Whitespace · Pivot + Run Compliance + Share Brief) from `5b1ac6e9c6` — same scaffolding (`AppShell`/`useApp`/`authToken`) so it's a drop-in — plus the token-gated public `BrandIntelligenceBriefPage` (`/brand-intelligence/:token`). Wiring: `main.tsx` imports + `/app/strategy-intel` (AppProvider) + public `/brand-intelligence/:token` (no auth, beside `/review/:token`); Sidebar nav "Brand Intelligence" (compass) + NAV_ROUTES; TopBar titles. Share contract verified end-to-end (POST /share → `https://<host>/brand-intelligence/<token>` → public route → brief page → GET /brief/:token; 410 + "This brief link has been revoked" on revoke). `tsc` clean; 199/199 tests.

### #483 — Authenticity Enricher "Re-run Enrichment" was a dud
Button was `onClick={() => setResult(null)}` — cleared the view, never called `runAnalysis`. Even a manual re-run would've lost `topicBriefId` (read from URL, cleaned after each run) and served the cached stale-brain result (no `force`). New `rerunEnrichment()` re-puts the current result's `topicBriefId` in the URL, then `runAnalysis(false, true)`; server's per-brief smart-nuke rebuilds just that brief on the current brain version. Added `topicBriefId` to the `EnrichResult` type (server already spread it into every payload).

### #485 — Brand-profile hallucination when a site can't be scraped (oooagency.com)
Bright Data Unlocker returned **HTTP 200 with a 0-byte body**, counted as success, and `looksLikeSpaShell('')` is false so it never escalated to the browser tier → the empty scrape flowed into a full profile whose voice/messaging Opus invented. Three fixes: (1) empty-body 200 logs `success:false` + returns failure → auto mode escalates; forgeScrape also escalates on any body <500 chars. (2) New `extractEmbeddedStateText` tier — Cargo/Next/Nuxt sites keep all copy in a `__PRELOADED_STATE__`/`application/json` blob Readability can't see (oooagency = 81KB HTML, 0 extractable); balanced-brace scan harvests the prose (verified: recovered their real copy). (3) On genuine scrape failure the prompt + a deterministic server guard **withhold** voiceProfile + messaging (`insufficientData:true`) instead of fabricating; banner reworded from "less accurate" to "withheld".

### #489 — Publish showed "Live" for LinkedIn with no View-post link
A duplicate LinkedIn publish raced past the alreadyPublished check 6s after the real Zernio publish, failed, fell to the legacy "not yet authorized" path, and wrote a `staged` shadow `publish_log` row. Three compounding defects: (1) `/sync` + `/log` picked newest-per-channel by `attempted_at` → shadow outranked the real published row; (2) `/sync` defaulted `live_status` to 'published' for any newest row → branded a never-live staged row as Live; (3) `persistResults` merged the url-less staged result over the good url-bearing `publish_results.linkedin`. Fixes: published rows outrank staged/error in both DISTINCT ON orderings; non-published rows reported as-is and skipped from live checks; a non-published result never clobbers a url-bearing entry. Repaired the affected article's data directly in prod.
...
## WORKING-STATE sessions log (archived from WORKING-STATE.md)

*(The accumulated session log moved out of WORKING-STATE.md so it stays the lean ~100-line pointer it is meant to be.)*
...
Full detail in `BUILD-HISTORY.md` (2026-06-09 and cont.). The base everything above builds on:
...
_Older sessions (2026-06-07 GEO arc, 2026-06-06 and earlier) archived in `BUILD-HISTORY.md`._
