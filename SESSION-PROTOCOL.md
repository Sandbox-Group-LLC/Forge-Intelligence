...
This document codifies how a Claude (or any AI agent) session should operate against the Forge Intelligence repo. Read at the start of any session, alongside `CLAUDE.md` (for code-graph context) and `WORKING-STATE.md` (for in-flight work).

Last refreshed for the current local-edit + git + draft-PR workflow.
...
3. **`WHITEBOARD.md`** (repo root) — long-form retrospective archive. Search by date or topic when context is missing.
