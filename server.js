...
async function ensureComplianceColumns(tableName) {
  const cols = [
    'review_mode TEXT DEFAULT 'approve_to_ship'',
  ...
  for (const col of cols) {
    ...
    await pool.query(`ALTER TABLE ${tableName} ADD COLUMN IF NOT EXISTS ${colName} ${col.split(' ').slice(1).join(' ')}`).catch(() => {});
  }
}

// POST /api/compliance/critique — AI self-critique of a generated article
app.post('/api/compliance/critique', async (req, res) => {
  ...
  try {
    ...
    // Load article
    const contentRes = await pool.query(`SELECT * FROM ${tableName} WHERE id = $1`, [contentId]);
    ...
    // Load system prompt
    const fs = require('fs');
    const systemPrompt = fs.readFileSync('./src/agents/stage5_compliance_gate/system_prompt.md', 'utf8');

    const reviewMode = content.review_mode || 'approve_to_ship';

    const userMessage = `Review Mode: ${reviewMode.toUpperCase()}

Brand Voice Profile:
${JSON.stringify(profile.voice_profile || {}, null, 2)}

Known Brain Mistakes:
${JSON.stringify(mistakes.map(m => ({ type: m.mistake_type, feedback: m.human_feedback, guardrail: m.guardrail_created })), null, 2)}

Article to Review:
Title: ${content.title}
${JSON.stringify(content.article_json?.sections || [], null, 2)}`;
    ...
    try {
      ...
      return res.status(500).json({ error: 'Failed to parse compliance report' });
    }

    // Save report to DB
    await pool.query(
      `UPDATE ${tableName} SET compliance_report = $1, compliance_status = $2, review_mode = $3, updated_at = NOW() WHERE id = $4`,
      [JSON.stringify(report), report.complianceStatus || 'needs_review', reviewMode, contentId]
    );

    await pool.query(
      `INSERT INTO agent_activity_log (agent_name, brand_profile_id, status, tokens_used, latency_ms, metadata)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      ['compliance_agent', brandProfileId, 'complete',
       (response.usage?.input_tokens || 0) + (response.usage?.output_tokens || 0),
       0, JSON.stringify({ contentId, reviewMode, flagCount: report.flags?.length || 0 })]
    ).catch(() => {});

    res.json({ success: true, report, contentId });
...
// POST /api/compliance/approve — save human edits, write mistakes to Brain, mark approved
app.post('/api/compliance/approve', async (req, res) => {
  const { brandProfileId, contentId, humanEdits, approvedSections, finalStatus, reviewMode } = req.body;
  if (!brandProfileId || !contentId) return res.status(400).json({ error: 'brandProfileId and contentId required' });
  ...
  try {
    ...
    // Write human edits to Brain mistakes table
    if (humanEdits && humanEdits.length > 0) {
      for (const edit of humanEdits) {
        if (edit.originalText && edit.editedText && edit.originalText !== edit.editedText) {
          const mistakeId = `mistake_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
          await pool.query(
            ...
            [
              mistakeId,
              'human_edit',
              `Original: "${edit.originalText}" → Edited: "${edit.editedText}" (Section: ${edit.sectionHeading || 'unknown'})`,
              `Avoid the phrasing: "${edit.originalText}". Prefer: "${edit.editedText}"`,
    ...
    // Update article with approvals + edits
    await pool.query(
      `UPDATE ${tableName}
       SET compliance_status = $1, approved_sections = $2, human_edits = $3,
           review_mode = $4, reviewed_at = NOW(), reviewed_by = $5, updated_at = NOW()
       WHERE id = $6`,
      [
        finalStatus || 'approved',
        JSON.stringify(approvedSections || []),
        JSON.stringify(humanEdits || []),
    ...
    res.json({ success: true, contentId, status: finalStatus || 'approved', mistakesWritten: humanEdits?.filter(e => e.originalText !== e.editedText).length || 0 });
...
// GET /api/compliance/content/:brandProfileId — list all generated content for a brand
app.get('/api/compliance/content/:brandProfileId', async (req, res) => {
  const { brandProfileId } = req.params;
  try {
    const safeId = brandProfileId.replace(/-/g, '_');
    const tableName = `generated_content_${safeId}`;
    const result = await pool.query(
      `SELECT id, title, overall_confidence, brain_match_score, hero_image_url,
              compliance_status, review_mode, reviewed_at, created_at, status
       FROM ${tableName} ORDER BY created_at DESC LIMIT 50`
    );
