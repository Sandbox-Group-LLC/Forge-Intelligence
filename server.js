...
Return ONLY valid JSON, no explanation:
{
  "rules": [
    ...
    let extracted = { rules: [] };
    try { extracted = safeParseLLM(clean); } catch(e) { console.error('[BRAIN-DISTILL] JSON parse error:', e.message, rawText.slice(0, 200)); }
...
Generate exactly ${numEmails} emails. Return ONLY valid JSON matching the output format.`;
    ...
    let parsed;
    try {
      parsed = safeParseLLM(jsonMatch ? jsonMatch[0] : raw);
    } catch(e) {
...
Return ONLY valid JSON:
{
  ...
  "briefRationale":"string"
    ...
    await pool.query('INSERT INTO agent_activity_log (agent_name, brand_profile_id, status, tokens_used, latency_ms, metadata) VALUES ($1,$2,$3,$4,$5,$6)', ['stage2_geo_strategist', brandProfileId, 'success', 0, latencyMs, JSON.stringify({ brandProfileId })]).catch(e => console.error('[ACTIVITY LOG]', e.message));
            res.json({ success: true, cached: false, data: {
...
{"enrichedTitle":"","enrichedH1":"","enrichedSections":[{"heading":"","eeatInjections":[],"confidenceFlag":"green|yellow|red","flagReason":null,"smeRequired":false}],"enrichedFAQ":[{"q":"","a":"","eeatSignal":""}],"authorSchemaMarkup":{},"overallConfidence":0,"readyForStage4":true,"humanReviewItems":[]}` },
    ...
    await pool.query('INSERT INTO agent_activity_log (agent_name, brand_profile_id, status, tokens_used, latency_ms, metadata) VALUES ($1,$2,$3,$4,$5,$6)', ['stage3_authenticity_enricher', brandProfileId, 'success', 0, latencyMs, JSON.stringify({ brandProfileId })]).catch(e => console.error('[ACTIVITY LOG]', e.message));
            res.json({ success: true, cached: false, data: {
...
Return ONLY valid JSON matching the output format. No markdown, no commentary.`;
    ...
    const raw = message.content[0].text;
    const jsonMatch = raw.match(/\{[\s\S]*\}/);
    const plan = safeParseLLM(jsonMatch ? jsonMatch[0] : raw);
