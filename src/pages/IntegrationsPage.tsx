...
const CHANNELS: ChannelDef[] = [
  ...
  },
  {
    ...
    description: 'Publish articles to your company Facebook Page. Recommended: paste a Pipedream workflow URL (Pipedream String lets you bring your own OAuth scopes). Legacy: paste a Page Access Token directly.',
    color: '#1877F2',
    logo: 'f',
    liveStatus: 'live',
    credentialFields: [
      { key: 'pipedreamWorkflowUrl', label: 'Pipedream Workflow URL (recommended)', placeholder: 'https://eo*.m.pipedream.net/...' },
      { key: 'pageId', label: 'Page ID (required for both paths)', placeholder: '123456789012345' },
      { key: 'pageAccessToken', label: 'Page Access Token (legacy fallback only)', placeholder: 'EAABsbCS... — leave blank if using Pipedream workflow', type: 'password' },
    ],
    setupGuide: {
      title: 'Connect Facebook via Pipedream Workflow (recommended)',
      steps: [
        { text: 'Step 1 — Create a Pipedream workflow. In your Pipedream account, create a new HTTP-triggered workflow. The chat engine connection builder can scaffold this for you in seconds.', url: 'https://pipedream.com/' },
        { text: 'Step 2 — Use Pipedream String (Alpha) to register a custom Facebook OAuth app with the scopes you actually need: pages_manage_posts, pages_read_engagement, pages_show_list, business_management, pages_manage_metadata, and any others your workflow requires. The default facebook_pages app has fixed scopes — String lets you bring your own.' },
        { text: 'Step 3 — Add a step in the workflow that uses your custom OAuth app to call POST https://graph.facebook.com/v21.0/{pageId}/feed with the body fields Forge sends: message, link. The pageId comes from the request body too (forgeMeta.pageId) so the same workflow can serve multiple brands.' },
        { text: 'Step 4 — Have the workflow respond with JSON: { success: true, postId: "<page-id>_<post-id>", url: "https://facebook.com/<post-id>" }. On error: { error: "<message>" } with non-2xx status. Forge logs success/failure based on this.' },
        { text: 'Step 5 — Copy the Pipedream workflow trigger URL (https://eo*.m.pipedream.net/...) and paste it above. Also paste your Facebook Page ID (find it on your Page under About → Page transparency).' },
        { text: 'Legacy direct-Graph fallback: leave the Pipedream URL blank and paste a long-lived Page Access Token in the field below. Tokens expire every ~60 days and need manual refresh — Pipedream is recommended for that reason alone.' },
      ],
    },
  },
  {
