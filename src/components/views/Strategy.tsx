  ...
  return (
    <div className="strategy">
      <div className="strategy-header">
        ...
        <div className="strategy-actions-wrap" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button className="strategy-action-btn strategy-action-count" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 36, padding: '0 16px', fontSize: 13, fontWeight: 600, border: '1px solid #e2e8f0', borderRadius: 'var(--radius-sm)', background: '#fff', color: '#334155', lineHeight: 1, boxSizing: 'border-box', fontFamily: 'inherit', cursor: 'default' }} disabled>
            {brandProfile.strategicRecommendations.length} Recommendations
          </button>
          <button className="strategy-action-btn" onClick={() => { const blob = new Blob([JSON.stringify(brandProfile, null, 2)], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${brandProfile.brandName || 'strategy'}-brief.json`; a.click(); }} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px', height: 36, padding: '0 16px', fontSize: 13, fontWeight: 500, border: '1px solid #e2e8f0', borderRadius: 'var(--radius-sm)', background: '#fff', color: '#64748b', cursor: 'pointer', textDecoration: 'none', lineHeight: 1, boxSizing: 'border-box', margin: 0, fontFamily: 'inherit' }}>
            <span className="btn-icon-decorative">{icons.download}</span> Export Brief
          </button>
          <button className="strategy-action-btn strategy-action-btn-primary" onClick={() => { window.location.href = '/app/geo-strategist'; }} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px', height: 36, padding: '0 16px', fontSize: 13, fontWeight: 600, borderRadius: 'var(--radius-sm)', background: '#4F46E5', color: '#fff', border: '1px solid #4F46E5', cursor: 'pointer', textDecoration: 'none', lineHeight: 1, boxSizing: 'border-box', margin: 0, fontFamily: 'inherit' }}>
            <span className="btn-icon-decorative"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg></span> Run GEO Strategy →
          </button>
        </div>
      </div>
