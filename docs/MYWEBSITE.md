...
- **No outer wrapper.** `html` is a flat sequence of content blocks — no `<html>`, `<head>`, or `<article>` wrapper, and no JSON-LD/schema. The only tags that appear: `<h2>` section headings, `<p>` paragraphs, `<ul>`/`<ol>`/`<li>` lists, inline `<strong>`/`<em>`/`<a>`, plus the TL;DR `<aside>` and FAQ `<section>` described below. `markdown` is the same content as `## heading` + paragraphs + `-`/`1.` lists. Your template owns the `<head>`, canonical tag, OG/meta, and structured data.
- **Lists arrive render-ready.** Section bodies with bullet or numbered lists ship as real `<ul>`/`<ol>` markup in `html`, and in `markdown` list blocks are always separated from preceding text by a blank line (so strict CommonMark renderers make actual lists, not run-on prose). You do not need any conversion code — store and render the body as-is.
...
| Lists render as a run-on paragraph of dashes | The article was published **before 2026-07-04**, when section bodies shipped as a single `<p>` (html) / without blank lines before list blocks (markdown). Current publishes send real `<ul>`/`<ol>` markup and properly delimited markdown. **Republish the article from Forge's Publishing Queue** — the upsert-by-slug overwrites the stored copy. If you can't republish, convert on your side: in `html`, split the `<p>` on newlines and wrap consecutive `- `/`1. ` lines in `<ul>`/`<ol>`; in `markdown`, insert a blank line before each list block. |
